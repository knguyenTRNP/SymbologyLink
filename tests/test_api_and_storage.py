...
class DecisionTests(unittest.TestCase):
    def test_rule_match_and_override_precedence(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(overridden.mappingVersion, "review-v2")
    ...
    def test_single_batch_idempotency_pagination_and_rules(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            with patch.dict(os.environ, environment):
                ...
                self.assertEqual(single.json()["matchedEntity"]["entityId"], "entity:microsoft")
                ...
                self.assertEqual(job_results["results"][0]["relationshipGraph"]["directParent"]["entityId"], "entity:microsoft")
                metrics = client.get(f"/v1/jobs/{job_id}/metrics").json()
                self.assertEqual(metrics["relationshipStatusCounts"]["resolved"], 1)
                self.assertIn("securityDecisionStatusCounts", metrics)
                self.assertIn("averageSecurityConfidence", metrics)
                self.assertIn("identifierConflictRecords", metrics)
                self.assertIn("identifierConflictCounts", metrics)
                self.assertEqual(metrics["relationshipPointInTimeVerified"], 0)
                self.assertEqual(metrics["temporalScopeStatusCounts"]["entity"]["not_requested"], 1)
                self.assertEqual(metrics["temporalScopeStatusCounts"]["relationships"]["not_requested"], 1)
